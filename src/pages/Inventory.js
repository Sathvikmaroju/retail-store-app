...
const handleAddProduct = async (e) => {
  ...
  const renderAccordion = (data) => (
    data.map((product) => {
      const isLowStock = (product.remainingQty || 0) < (product.lowStockThreshold || 10);
            ...
            {isEditing ? (
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 2 }}>
                ...
                <Autocomplete
                  ...
                  onChange={(event, newValue) => handleProductEdit(currentProduct, 'category', newValue || '')}
                  onInputChange={(event, newInputValue) => handleProductEdit(currentProduct, 'category', newInputValue)}
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      label="Category"
                ...
                <Autocomplete
                  ...
                  onChange={(event, newValue) => handleProductEdit(currentProduct, 'subCategory', newValue || '')}
                  onInputChange={(event, newInputValue) => handleProductEdit(currentProduct, 'subCategory', newInputValue)}
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      label="Sub Category"
                ...
                <TextField
                  ...
                  onChange={(e) => handleProductEdit(currentProduct, 'unitType', e.target.value)}
                  size="small"
                ...
                <TextField
                  ...
                  onChange={(e) => handleProductEdit(currentProduct, 'purchasePricePerUnit', e.target.value)}
                  size="small"
                />
                <TextField
                  label="Vendor Name"
                ...
                <TextField
                  ...
                  onChange={(e) => handleProductEdit(currentProduct, 'vendorDetails', e.target.value, true, 'vendor_desc')}
                  size="small"
                  multiline
                  rows={2}
                ...
                {/* Action Buttons */}
                <Box sx={{ gridColumn: 'span 2', display: 'flex', gap: 1, justifyContent: 'flex-start', mt: 2 }}>
                  <Button
                    variant="contained"
                    color="primary"
                    startIcon={<SaveIcon />}
              ...
              // View Mode
              <Box>
                <Typography><strong>Category:</strong> {currentProduct.category} / {currentProduct.subCategory || '-'}</Typography>
                <Typography><strong>Unit:</strong> {currentProduct.unitType}</Typography>
                <Typography><strong>Selling Price per Unit:</strong> ₹{currentProduct.pricePerUnit}</Typography>
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      {/* Products Display */}
      <Typography variant="h5" color="green" gutterBottom>In Stock</Typography>
      {renderAccordion(inStock)}

      <Divider sx={{ my: 3 }} />

      <Typography variant="h5" color="error" gutterBottom>Out of Stock</Typography>
